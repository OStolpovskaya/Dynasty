        ...
        if (!Objects.equals(dropResName, "") && !Objects.equals(takeResName, "")) {
            if (!dropResName.equals(takeResName)) {
                ...
                if (10 <= count && count <= dropResCount) {
                    ...
                    logger.info(family.userNameAndFamilyName() + " exchanges " + floorCount + " of resource " + dropRes + " to " + newResAmount + " of resource " + takeRes);
                    mess = "Вы обменяли " + floorCount + " шт. ресурса " + dropResName + " на " + newResAmount + " шт. ресурса " + takeResName;
                } else {
                    logger.error(family.userNameAndFamilyName() + " not enough resources to exchange:" + dropRes);
                    mess = "Выбранного для обмена ресурса должно быть больше 10, но меньше или равно, чем есть у вас";
                }
            } else {
                logger.debug(family.userNameAndFamilyName() + " try to exchange the same resources:" + dropRes);
                mess = "Невыгодно обменивать один и тот же ресурс.";
            }
        } else {
            logger.error(family.userNameAndFamilyName() + " try to exchange nonexisting resource:" + dropRes);
