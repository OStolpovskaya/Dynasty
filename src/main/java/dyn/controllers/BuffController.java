...
public class BuffController {
    ...
    public String chooseBuffs(ModelMap model, RedirectAttributes redirectAttributes,
        ...
        logger.error(family.userNameAndFamilyName() + "'s character " + characterId + " is not belongs to user's current family fiances");
        ...
        if (character != null) {
            ...
            if (!character.getBuffs().contains(buff) && !character.getBuffs().contains(buff.getContradictory())) {
                if (family.getMoney() >= buff.getCost()) {
                    ...
                    logger.debug(family.userNameAndFamilyName() + "'s character " + characterId + " now has buff " + buff.getTitle());
                    ...
                    logger.error(family.userNameAndFamilyName() + ": not enough money for buff " + buff.getTitle());
                    return "redirect:/game";
                }
                ...
                logger.error(family.userNameAndFamilyName() + "'s character " + characterId + " has this buff or contradictory to this buff");
                return "redirect:/game";
            }
    ...
    public String applyItemSkill(ModelMap model, RedirectAttributes redirectAttributes,
        ...
        if (item != null) {
            ...
            if (character != null) {
                ...
                if ((levelCheck && (sonCheck || daughterCheck)) || (wifeOfSonCheck && (character.getSpouse().getLevel() == family.getLevel()))) {
                    ...
                    if (projectId.equals(Const.PROJECT_SKILL_INTELLIGENCE)) {
                        ...
                        if (!character.getBuffs().contains(buff) && !character.getBuffs().contains(buff.getContradictory())) {
                            ...
                            logger.error(family.userNameAndFamilyName() + "want to apply buff, but character already has this buff or has contradictory buff: " + buff.getId());
                ...
                logger.error(family.userNameAndFamilyName() + "want to apply item to nonexisting character: " + characterId);
    ...
    public String applyItemParents(ModelMap model, RedirectAttributes redirectAttributes,
        ...
        if (item != null) {
            ...
            if (character != null) {
                ...
                if (!canHaveChild) {
                    logger.error(family.userNameAndFamilyName() + " want to apply parents buff, but reached max children: " + item.getProject().getId());
                ...
                if (levelCheck && familyCheck && hasChildren) {
                    ...
                    logger.error(family.userNameAndFamilyName() + "want to apply item to not right character: " + character.getMainDetails() + ". Условия: " + levelCheck + "," + familyCheck + "," + hasChildren);
                    redirectAttributes.addFlashAttribute("mess", "Условия применения не подходят к выбранному персонажу " + character.getFullName());
                    return "redirect:/game";
                }
            } else {
                ...
                if ((levelCheck && (sonCheck || daughterCheck)) || (wifeOfSonCheck && (character.getSpouse().getLevel() == family.getLevel()))) {
                    ...
                    if (projectId.equals(Const.PROJECT_VOCATION_CHANGE)) {
                        ...
                        if (vocation != null) {
                            ...
                            logger.error(family.userNameAndFamilyName() + "want to change vocation to nonexisting vocation: " + param);
